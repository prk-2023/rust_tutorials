...
- Every pattern must be handled exhaustively either explicitly or by using wildcards like `_` in the `match`
  Since `match` is an expression, values can also be returned.
...
For more information on `match` and matching in general: [Match Reference](https://doc.rust-lang.org/stable/reference/expressions/match-expr.html).
