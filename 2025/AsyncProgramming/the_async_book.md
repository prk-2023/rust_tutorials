...
### * Parallelism: 
  This is about executing multiple tasks simultaneously which often requires multiple threads or even
  multiple CPU cores. 
...
- Event Loop: 
  ...
  tasks that are waiting (such as I/O bound tasks) and switch between then without blocking the main
  program. When One task is waiting ( ex: network data) the event loop moves on to run the next ready task.


- Futures and Tasks: 
    This is basic building blocks in async programming **future** and **task**.
...
2. Async I/O (Non-blocking):
    ...
    So, you can handle many more requests with the same amount of resources.

## Rust approach:

- ***Futures** and **async/await** in Rust are designed to work with a single-threaded or multi-threaded 
  event loop to manage concurrent tasks efficiently. 
  The compiler’s async system transforms async functions into state machines, which can then be polling by 
  the runtime.

  The async runtime (like smol, tokio, or async-std) is responsible for managing the event loop and running 
  the tasks, switching between them when necessary.

  Rust’s ownership and borrowing system also makes async programming safe, even when tasks share data. 
  Since Rust ensures that no data races occur (through ownership rules), async programming in Rust is 
  concurrent and safe by design.
...
Also Async programming also offers the programmer fine-grain control over how tasks are executed ( level of
parallelism and concurrency, control flow, scheduling and so forth ). => Async programming can be expressive
ad well as ergonomic for many uses. 
